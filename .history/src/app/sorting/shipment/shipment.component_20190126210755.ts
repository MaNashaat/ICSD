...
export class ShipmentComponent implements OnInit, OnDestroy {
  ...
  get ShipmentDate() {
    if (this.item.ShipmentDt != null && this.item.ShipmentDt !== undefined) {
      const day = new Date(this.item.ShipmentDt).getUTCDate();
      const month = new Date(this.item.ShipmentDt).getUTCMonth() + 1;
      const year = new Date(this.item.ShipmentDt).getUTCFullYear();
