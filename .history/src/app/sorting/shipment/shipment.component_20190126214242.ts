...
export class ShipmentComponent implements OnInit, OnDestroy {
  ...
  set ShipmentDate(val: any) {
    if (val != null && val !== undefined) {
      this.item.ShipmentDt = new Date(val.year, val.month, val.day);
    }
  }
  get ShipmentDate() {
    const shipDate = new Date(this.item.ShipmentDt);
    return {
      year: shipDate.getUTCFullYear(),
      month: shipDate.getUTCMonth() + 1,
      day: shipDate.getUTCDate()
    };
    /* return {
      year: this.item.ShipmentDt == null ? null : new Date(this.item.ShipmentDt).getFullYear(),
      month: this.item.ShipmentDt == null ? null : +new Date(this.item.ShipmentDt).getMonth() + 1,
      day: this.item.ShipmentDt == null ? null : new Date(this.item.ShipmentDt).getDate()
    }; */
  }
