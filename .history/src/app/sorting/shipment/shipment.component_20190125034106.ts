...
export class ShipmentComponent implements OnInit, OnDestroy {
  ...
  searchVend = (text$: Observable<string>) => {
    ...
    return merge(debouncedText$, inputFocus$, clicksWithClosedPopup$).pipe(
      map(term => (term === ''
      ? this.vendorLst
      : this.vendorLst.filter(v => v.Name.toLowerCase().indexOf(term.toLowerCase()) > -1)).slice(0, 10))
    );
  }
  ...
  searchCustomer = (text$: Observable<string>) => {
    ...
    return merge(debouncedText$, inputFocus$, clicksWithClosedPopup$).pipe(
      map(term => (term === ''
      ? this.customerLst
      : this.customerLst.filter(v => v.Name.toLowerCase().indexOf(term.toLowerCase()) > -1)).slice(0, 10))
    );
  }

  selectedItem(item) {
    this._CustomerService.getAddressesByCustomer(item).subscribe((result) => {
      this.item.Address = result[0].Address;
      this.item.CustomerAddress_Id = result[0].ID;
    });
  }
