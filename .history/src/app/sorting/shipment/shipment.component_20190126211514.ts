...
export class ShipmentComponent implements OnInit, OnDestroy {
  ...
  get ShipmentDate() {
    if (this.item.ShipmentDt != null && this.item.ShipmentDt !== undefined) {
      const day = new Date(this.item.ShipmentDt).getDate();
      const month = +new Date(this.item.ShipmentDt).getMonth() + 1;
      const year = new Date(this.item.ShipmentDt).getFullYear();
