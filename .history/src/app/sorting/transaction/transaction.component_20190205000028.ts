...
export class TransactionComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.transLstSub = this._ShipmentService.getAllByDate().subscribe(result => {
      ...
      this.list.forEach(x => {
        if (!x.Transaction) {
          x.Transaction = { ID: 0, DeliveryStatus: null, IDNum: '', IDType: null, Notes: '' };
        }
      });
    });
  ...
  save() {
    ...
    this.list.forEach(p => {
      transactionlst.push({
        ...
        Shipments_Id: p.ID,
        DeliveryStatus: p.Transaction.DeliveryStatus[0],
        DeliveryStatuses_Id: p.Transaction.DeliveryStatus ? p.Transaction.DeliveryStatus[0].ID : 0,
        IDTypes_Id: p.Transaction.IDType ? p.Transaction.IDType[0].ID : 0,
        IDType: p.Transaction.IDType,
